...
export const payFail = (payOrderInfo, resultMsg) => {
  ...
  } else if (resultMsg === '支付参数不完整' || resultMsg === '支付参数错误') {
    // 支付参数错误，提示用户配置问题
    Toast({
      context: this,
      selector: '#t-toast',
...
export const wechatPayOrder = (payOrderInfo) => {
  try {
    ...
    const { timeStamp, nonceStr, signType, paySign, package: pkg } = payInfo;
    ...
    // 如果解析失败或参数不完整，显示错误提示
    payFail(payOrderInfo, error.message || '支付参数错误');
