...
Page({
  ...
  async loadGoodsList() {
    ...
    try {
      ...
      const images = goodsList.map((x) => x.cover_image).filter(img => img);
      
      if (images.length > 0) {
        const handledImages = await getCloudImageTempUrl(images);
        handledImages.forEach((image, index) => {
          const imgIndex = goodsList.findIndex((x) => x.cover_image === images[index]);
          if (imgIndex >= 0) {
            goodsList[imgIndex].cover_image = image;
          }
        });
      }
