...
// 处理富文本中的图片，使其自适应容器宽度
function processRichTextImages(html) {
  if (!html) return '';
  
  // 移除图片上的固定宽高样式，改为自适应
  let processed = html.replace(/<img([^>]*)style="([^"]*)"([^>]*)>/gi, (match, before, style, after) => {
    // 移除固定宽高，保留其他样式
    const newStyle = style
      .replace(/width\s*:\s*[^;]+;?/gi, '')
      .replace(/height\s*:\s*[^;]+;?/gi, '')
      .trim();
    const adaptiveStyle = 'max-width:100%;height:auto;display:block;' + (newStyle ? newStyle : '');
    return `<img${before}style="${adaptiveStyle}"${after}>`;
  });
  
  // 处理没有 style 属性的 img 标签
  processed = processed.replace(/<img(?![^>]*style=)([^>]*)>/gi, (match, attrs) => {
...
Page({
  ...
  async addCart({ detail: { count, pickedSku } }) {
    ...
    if (cartItem == null) {
      ...
      const finalCount = (cartItem.quantity || 0) + count;
  ...
  async getInfo(spuId) {
    ...
    if (!spu) {
      console.error('Failed to fetch SPU');
    ...
    records.forEach((x) => {
      x.userNameString = x.userName ? x.userName.substring(0, 10) : '用户';
    });

    // 处理富文本：替换云图片链接 + 图片自适应
    let detail = await replaceCloudImageWithTempUrl(spu.detail || '');
    detail = processRichTextImages(detail);

    // 确保 swiper_images 是数组
    let images = spu.swiper_images;
    if (!Array.isArray(images)) {
      images = [];
    }
    // 如果没有轮播图但有封面图，使用封面图
    if (images.length === 0 && spu.cover_image) {
      images = [spu.cover_image];
    }
